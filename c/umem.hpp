...
namespace umem {
  ...
  public:
    Address(uintptr_t adr, alignment_t alignment,  size_t size, void * raw_ctx, bool own = false):
      adr(adr), alignment(alignment), size(size), raw_ctx(raw_ctx), own(own) {
      std::cout << "  Address::Address("<<adr<<", "<<alignment<<", "<<size<<", ctx="<<raw_ctx<<", own="<<own<<")\n";
    }
    ~Address() {
      std::cout << "  Address::~Address()["<<adr<<", "<<alignment<<", "<<size<<", ctx="<<raw_ctx<<", own="<<own<<"]\n";
      if (own) {
    ...
    inline char& operator[] (size_t n) { return ((char*)adr)[n]; } // TODO: ensure n<size
    inline void set(int c, size_t nbytes) { umem_set(raw_ctx, adr, c, nbytes); }; // TODO: ensure nbytes<size
    ...
    //inline bool operator != (uintptr_t i) const { return adr != i; }
    inline Address operator + (int i) const { return Address(adr + i, alignment, (i<(int)size ? size - i : 0), raw_ctx); }
    inline Address operator + (size_t i) const { return Address(adr + i, alignment, (i<size ? size - i : 0), raw_ctx); }
    //inline Address operator - (int i) const { return Address(adr - i, alignment, size + i, raw_ctx); }
    //inline Address operator - (size_t i) const { return Address(adr - i, alignment, size + i, raw_ctx); }
    inline int operator % (int i) const { return adr % i; }
    inline size_t operator % (size_t i) const { return adr % i; }
    inline Address origin() const { return (alignment ? Address(umem_aligned_origin(raw_ctx, adr), 0, size, raw_ctx) : Address(adr, alignment, size, raw_ctx)); }
